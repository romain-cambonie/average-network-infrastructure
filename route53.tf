resource "aws_route53_zone" "average" {
...
resource "aws_route53_record" "main_name_servers_record" {
  ...
  records         = aws_route53_zone.average[each.key].name_servers
}
...
resource "aws_route53_record" "certificate_validation_main" {
  ...
  zone_id         = aws_route53_zone.average[each.key].id
  type            = "CNAME"
  ttl             = "300"
...
resource "aws_route53_record" "average_record_ipv4" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.average.domain_name
    zone_id                = aws_cloudfront_distribution.average.hosted_zone_id
